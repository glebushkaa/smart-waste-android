...
class GetRecyclePointsByCategoriesUseCase @Inject constructor(
    ...
    override suspend fun invoke() = runCatching {
        withContext(Dispatchers.IO) {
            val slugs = bucketRepository.getCurrentBucketCategorySlugs()
            recycleRepository.getRecyclePointsByCategories(categories = slugs)
        }
    }
}
